...
def parse_bruker_zip(zip_bytes: bytes):
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as z:
        names = z.namelist()
        fid_files = [f for f in names if f.replace('\\', '/').split('/')[-1] == 'fid']
        acqus_files = [f for f in names if f.replace('\\', '/').split('/')[-1] in ('acqus', 'acqu')]
        ...
        if ml1 != 0 and ml2 != 0:
            radicand = np.maximum(0, ml1**2 + 4 * ml2 * (time_axis * 1e6 - ml3))
            mz = ((np.sqrt(radicand) - ml1) / (2 * ml2)) ** 2
        else:
...
def load_spectrum_data(uploaded_file):
    """Parsea .mzML, .txt, .csv, .tsv o .zip de Bruker asegurando compatibilidad."""
    ...
    if fname.endswith(".mzml"):
        ...
        # Ignora metadatos o cabeceras no numéricas
        valid_lines = [l for l in content.splitlines() if re.match(r'^\s*[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?[\s,;\t]', l)]
        if not valid_lines:
            raise ValueError("No se detectaron datos numéricos válidos en el archivo.")
            
        df = pd.read_csv(io.StringIO("\n".join(valid_lines)), sep=None, engine='python', header=None)
...
def preprocess_spectrum(mz, intensity, min_mz=2000, max_mz=12000, snip_iters=35, prominence=800):
...
# VISTA STREAMLIT
# ---------------------------------------------------------
st.title("🔬 MALDI-TOF : STEC O157:H7 vs. Shigella spp.")
...
if uploaded_files:
    ...
    for file in uploaded_files:
        try:
            ...
            if len(mz_win) == 0:
                st.warning(f"El archivo '{file.name}' no tiene suficientes puntos en el rango diagnóstico (2000-12000 m/z).")
                continue
